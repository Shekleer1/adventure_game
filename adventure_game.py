...
Now you got to choose whith way should you go...''')
    input('')
    choice = int(input('1. Take my way through an old cave passage.\n2. Enter laboratory\n ==> '))
    if choice == 1:
        return 'passage'
    elif choice == 2:
        return 'lab'
...
''')
    input('')
    weapon_shield(1, 0)
    choice = int(input(f'''two way's: 1. passage 2. empty room'''))
    if choice == 1:
        return 'passage'
    elif choice == 2:
        return 'empty room'
...
Haunting eyes pierced the darkness, revealing a tortured soul bound to the mortal realm, forever guarding ancient secrets with a spectral sword at the ready.''')
    choice = int(input("Would you dare take this fight?\n1. Take the fight 2.Return ==> "))
    if choice == 1:
        hp = Link.get('health')
        if fight(Link, Ghost_knight):
            Link.set('health', hp)
            print(f"After a fight Link fall in to a deep sleep and restore his HP")
            weapon_shield(1, 0)
        else:
            print(f'''R.I.P Link''')
    elif choice == 2:
